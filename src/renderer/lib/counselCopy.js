...
export function askPlaceholder({ allAgents, names, mode, discussing, held, guest, mayAsk } = {}) {
  ...
  // host's, and whether they may be asked is the host's to say: unless they have
  // said yes, the words go to the room, the people in it read them, and no agent
  // is disturbed. Naming them here — "Give Hermes and Tessie something to
  // discuss" — would be this box promising an answer that is not coming, which
  // is worse than saying nothing.
  //
  // When the host has said yes, the sentences below are exactly right as they
  // stand: the names in them are the host's cast, and asking is what typing now
  // does. A room with nobody to ask stays on the first sentence — there is a
  // permission and no one to spend it on.
  if (guest && (mayAsk !== true || list.length === 0)) return `Say something to the room…${keys}`;
